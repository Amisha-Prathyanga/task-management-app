...
$(document).ready(function() {
    ...
    $(document).on('click', '.markComplete', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                $(`#task-${taskId}`).find('.status').html('<span class="badge bg-success">Completed</span>');
                Swal.fire({
                    icon: 'success',
                    title: 'Success',
        ...
        return `
            <div class="card mb-3" id="task-${task.id}">
                <div class="card-body">
                    ...
                    <div class="mt-3">
                        <button class="btn btn-sm btn-primary editTask" data-task-id="${task.id}">Edit</button>
                        <button class="btn btn-sm btn-danger deleteTask" data-task-id="${task.id}">Delete</button>
                        <button class="btn btn-sm btn-success markComplete" data-task-id="${task.id}">Mark Complete</button>
                    </div>
                </div>
            </div>
        `;
